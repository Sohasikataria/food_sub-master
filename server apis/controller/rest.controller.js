...
exports.addreview = async(req,res,next)=>{
    try{
        ...
        const rest = await rstService.updatedbillboarduuser(id);
        const rest1 = await rstService.updatedbillboardrating(id,rating);
        const rest2 = await rstService.updatedrlist(id,review);
        res.status(200).json({status:true,rest:rest,message:"update in sucessfully"});
    } catch (e){
        console.log(e)
        res.json({status:false,sucess:"server error controller login"});
